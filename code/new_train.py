...
def _run_validation(sess, dev_batch_hypotheses, dev_batch_labels, dev_batch_explanations, dev_batch_img_features, 
                    batch_size, ilabel2itoken, model, keep_prob):
    """
    ...
    # get initial state using multimodal feature
    ...
    for t in range(model_config.padded_length):        
        if t == 0:
            # input start token
            explanations_in = dev_batch_explanations[:, 0].reshape(-1, 1)
            current_pred = explanations_in
        if t == 1:
            # prepend predicted label
            current_pred = []
            for i in labels:
                current_pred.append(ilabel2itoken[i])
        ...
        #print("Debug", model['initial_state'])
...
def _run_validation_perplexity(sess, dev_batch_hypotheses, dev_batch_labels, dev_batch_explanations, dev_batch_img_features, batch_size, ilabel2itoken, model, keep_prob):
    """
...
def main(_):
    ...
    with g.as_default():       
        ...
        model = build_model(model_config, embeddings, ilabel2itoken=ilabel2itoken, mode=mode)
        ...
        with tf.Session() as session:
            ...
            for epoch in range(training_config.total_num_epochs):
                ...
                for indexes in batch(dev_batches_indexes, FLAGS.dev_batch_size):
                    ...
                    pred_explanations, pred_labels = _run_validation(session, dev_batch_hypotheses, dev_batch_labels, dev_batch_explanations, dev_batch_img_features, len(indexes), ilabel2itoken, model, 1.0)
                    
                    if FLAGS.imbalance == True:
                        dev_batch_num_correct += np.dot(pred_labels == dev_batch_labels, class_weights[dev_batch_labels])
                    else:   
                        dev_batch_num_correct += (pred_labels == dev_batch_labels).sum()
                    dev_num_correct += dev_batch_num_correct

                    progress.update(dev_batch_index, [("Proportion of correct labels", float(dev_batch_num_correct)/ len(indexes))])
                    if FLAGS.sample_every > 0 and (t+1) % FLAGS.sample_every == 0:
                        ...
                        pred_explanations_decoded = [decode(pred_explanations[i], id2token) for i in range(len(indexes))]
                        print("\nExample generated explanation: ", pred_explanations_decoded[0]) #TODO: decode it
